  ...
  return <Sidebar className="border-r border-slate-200 bg-white">
      ...
      <SidebarContent className="bg-white">
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {mainMenuItems.filter(item => !item.hidden).map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton onClick={() => handleNavigation(item.page)} isActive={currentPage === item.page} className={`hover:bg-slate-100 cursor-pointer py-3 px-3 touch-manipulation ${currentPage === item.page ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600' : 'text-slate-700'}`}>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {adminMenuItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton onClick={() => handleNavigation(item.page, item.locked)} isActive={currentPage === item.page} className={`hover:bg-slate-100 cursor-pointer py-3 px-3 touch-manipulation ${currentPage === item.page ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600' : 'text-slate-700'}`}>
                    <div className="flex items-center gap-3">
                      <item.icon className="w-5 h-5 sm:w-4 sm:h-4" />
                      <span className="font-medium text-base sm:text-sm">{item.title}</span>
                    </div>
